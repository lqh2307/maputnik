...
export type HighlightedLayer = LayerSpecification & {
...
export function colorHighlightedLayer(
  layer?: LayerSpecification
): HighlightedLayer | null {
  if (!layer || layer.type === "background" || layer.type === "raster")
    return null;
  ...
  if (layer.type === "fill" || layer.type === "fill-extrusion") {
    return changeLayer(
      stylegen.polygonLayer(color, color, layer.source, layer["source-layer"]),
      layer
    );
  }
  ...
  if (layer.type === "symbol" || layer.type === "circle") {
    return changeLayer(
      stylegen.circleLayer(color, layer.source, layer["source-layer"]),
      layer
    );
  }
  ...
  if (layer.type === "line") {
    return changeLayer(
      stylegen.lineLayer(color, layer.source, layer["source-layer"]),
      layer
    );
  }
