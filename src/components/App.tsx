...
import { runtimeTokens } from "../config/runtime";
...
function setFetchAccessToken(url: string, mapStyle: StyleSpecification) {
  ...
  if (matchesTilehosting || matchesMaptiler) {
    const accessToken = getAccessToken("openmaptiles", mapStyle, {
      allowFallback: true,
    });
    if (accessToken) {
    ...
    const accessToken = getAccessToken("thunderforest", mapStyle, {
      allowFallback: true,
    });
    if (accessToken) {
      return url.replace("{key}", accessToken);
    ...
    const accessToken = getAccessToken("locationiq", mapStyle, {
      allowFallback: true,
    });
    if (accessToken) {
      return url.replace("{key}", accessToken);
...
export class App extends React.Component<any, AppState> {
  ...
  onStyleChanged = (
    ...
    const isEmptySources =
      !oldStyle.sources || Object.keys(oldStyle.sources).length === 0;
    ...
    if (newStyle && newStyle.layers) {
      const foundLayers = new globalThis.Map();
    ...
    const mappedErrors: MappedError[] = layerErrors
      .concat(errors)
      .map((error) => {
  ...
  async fetchSources() {
    ...
    for (const key of Object.keys(this.state.mapStyle.sources)) {
      ...
      if (source.type !== "vector" || !("url" in source)) {
        ...
        }) as SourceSpecification & { layers: string[] };
  ...
  mapRenderer() {
    ...
    return (
      <div className="maputnik-map__container" data-wd-key="maplibre:container">
        {mapElement}
      </div>
    );
  }
  ...
  render() {
    ...
    const toolbar = (
      <AppToolbar
        ...
        onToggleModal={(modal: keyof AppState["isOpen"]) =>
          this.toggleModal(modal)
        }
      />
    );

    const codeEditor = this.state.isOpen.codeEditor ? (
      <CodeEditor
