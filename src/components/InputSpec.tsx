...
export type InputSpecProps = {
  onChange?(
    ...
    value: number | undefined | (string | number | undefined)[]
  ): unknown;
  fieldName?: string;
  fieldSpec?: {
    default?: unknown;
  ...
  childNodes() {
    ...
    switch (this.props.fieldSpec?.type) {
      case "number":
        return (
          <InputNumber
            {...(commonProps as unknown as InputNumberProps)}
            min={this.props.fieldSpec.minimum}
            max={this.props.fieldSpec.maximum}
          />
        );
      case "enum": {
        const options = Object.keys(this.props.fieldSpec.values || []).map(
          (v) => [v, capitalize(v)]
        );

        return (
          <InputEnum
            {...(commonProps as unknown as Omit<InputEnumProps, "options">)}
            options={options}
          />
        );
      }
      ...
      case "string":
        if (iconProperties.indexOf(this.props.fieldName!) >= 0) {
          ...
          return (
            <InputString {...(commonProps as unknown as InputStringProps)} />
          );
        }
      case "color":
        return <InputColor {...(commonProps as unknown as InputColorProps)} />;
      case "boolean":
        return (
          <InputCheckbox {...(commonProps as unknown as InputCheckboxProps)} />
        );
      case "array":
        if (this.props.fieldName === "text-font") {
          return (
            <InputFont
              {...(commonProps as unknown as InputFontProps)}
          ...
          if (this.props.fieldSpec.length) {
            return (
              <InputArray
                {...(commonProps as unknown as InputArrayProps)}
            ...
            return (
              <InputDynamicArray
                {...(commonProps as unknown as InputDynamicArrayProps)}
                fieldSpec={this.props.fieldSpec}
                type={
                  this.props.fieldSpec.value as InputDynamicArrayProps["type"]
                }
              />
            );
          }
          ...
          <InputDynamicArray
            ...
            value={
              (Array.isArray(this.props.value)
                ? this.props.value
                : [this.props.value]) as (string | number | undefined)[]
            }
          />
        );
      case "colorArray":
      ...
      default:
        console.warn(
          `No proper field input for ${this.props.fieldName} type: ${this.props.fieldSpec?.type}`
        );
        return null;
    }
  }
